...
instance
  ...
  Path-Comp : ∀{ℓ} {A : Type ℓ} → Compω (Paths A)
  Path-Comp = {!!}
...
--     ∀ {t} (k : Hom t x) → α _ k ＝ k ∙ α _ refl
  ...
  module _
    ...
    push-ass : {w x y z : Ob _} (f : Hom w x) (g : Hom x y) (h : Hom y z)
             → f ∙ (g ∙ h) ＝ f ∙ g ∙ h
    push-ass f g h = {!!}

module _ {ℓ-ob ℓ-hom} (C : Quiverω 0 ℓ-ob ℓ-hom) (open Quiverω C)
  ⦃ _ : Compω C ⦄ where
  Assocp : ∀{lxs lws lys lzs} {x : Ob lxs} {w : Ob lws} (f : Hom x w) {y : Ob lys} (g : Hom w y) {z : Ob lzs} (h : Hom y z) → Type (ℓ-hom lxs lzs)
  Assocp f g h = f ∙ (g ∙ h) ＝ (f ∙ g) ∙ h

  Assocp2 : ∀{lys lzs} {y : Ob lys} {z : Ob lzs} (h : Hom y z) → Type (ℓ-ob tt ⊔ ℓ-hom tt tt)
  Assocp2 {y} {z} h = {x w : Ob _} (f : Hom x w) (g : Hom w y) → f ∙ (g ∙ h) ＝ (f ∙ g) ∙ h

  -- module _ ⦃ rc : Reflω C ⦄ where
  module _ ⦃ rc : Reflω C ⦄ ⦃ ex : Extendω C λ p → Paths (Assocp2 p) ⦄ ⦃ lex : Lawful-Extendω C λ p → Paths (Assocp2 p) ⦄ where

    wto : {x y z w : Ob _} {r : Hom w z} {q : Hom z x} (p : Hom x y) → Assocp2 refl → Assocp2 p
    wto = extend-l

    -- kak : {x : Ob tt}
    --         {u
    --          : {x = x₁ : Ob tt} {w : Ob tt} (f : Hom x₁ w) (g : Hom w x) →
    --            f ∙ (g ∙ refl) ＝ f ∙ g ∙ refl} →
    --         extend-l refl u ＝ extend-l refl u
    -- kak = extend-refl ⦃ rc ⦄ ⦃ ex ⦄ ⦃ lex ⦄
    ...
    tak : {!{x : Ob tt}
            {u
             : {x = x₁ : Ob tt} {w : Ob tt} (f : Hom x₁ w) (g : Hom w x) →
               f ∙ (g ∙ refl) ＝ f ∙ g ∙ refl} →
            u ＝ extend-l refl u!}
    tak = extend-coh ⦃ rc ⦄ ⦃ ex ⦄ ⦃ lex ⦄

    -- _ : ∀{lxs lys} {x : Ob lxs} {y : Ob lys} (p : Hom x y) → Hom y y → Hom x y
    -- _ = extend-r ⦃ rc ⦄ ⦃ ex ⦄

    -- beb : ∀{lxs lws lys lzs} {x : Ob lxs} {w : Ob lws} (f : Hom x w) {y : Ob lys} (g : Hom w y) {z : Ob lzs} (h : Hom y z) → f ∙ (g ∙ h) ＝ (f ∙ g) ∙ h
    -- beb {x = x} f g h = let uu = extend-refl ⦃ rc ⦄ ⦃ ex ⦄ ⦃ lex ⦄ {x = f} {u = {!!}} in {!!}
