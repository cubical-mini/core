{-# OPTIONS --safe #-}
module Foundations.Quiver.Lens.Extend.Display where
...
module _ {m ℓ-ob} {Ob : ob-sig ℓ-ob} {ℓ-hom}
  ...
  Disp± : ⦃ ex : Extend C k α ⦄ → HQuiver-onωᵈ C k (λ t → F (ex .Extend.rfl .refl)) _
...
module _ {m ℓ-ob} {Ob : ob-sig ℓ-ob} {ℓ-hom}
  ...
  ⦃ ex : Extend C k α ⦄ where instance

  Disp±-Reflᵈ : Reflᵈ (Disp± α)
  Disp±-Reflᵈ .Reflᵈ.rfl = ex .Extend.rfl
...
module _ {m ℓ-ob} {Ob : ob-sig ℓ-ob} {ℓ-hom}
  ...
  {α : ∀{ls} (t : Ob ls) → HQuiver-onω k (F t) (ℓ-homᶠ ls)} where instance
    ...
    Push→Extend : ⦃ _ : ∀{ls} {t : Ob ls} → Refl (α t) ⦄ ⦃ hp : HPush C k α ⦄
                → Extend C k (λ {y = y} _ → α y)
    Push→Extend ⦃ hp ⦄ .Extend.rfl = hp .Push.rfl
    Push→Extend .extend-l p u = u ▷ p
    Push→Extend .extend-r _ v = v
    Push→Extend .extend-refl = push-refl
    Push→Extend .extend-coh = refl

    Pull→Extend : ⦃ hp : HPull C k α ⦄ → Extend C k (λ {x = x} _ → α x)
    Pull→Extend ⦃ hp ⦄ .Extend.rfl = hp .Pull.rfl
