...
reflₚ : {ℓ : Level} {A : Type ℓ} {x : A} → x ＝ x
reflₚ {x} _ = x

symₚ : {ℓ : Level} {A : Type ℓ} {x y : A} → x ＝ y → y ＝ x
symₚ p i = p (i~ i)

ap : {ℓ ℓ′ : Level} {A : Type ℓ} {B : A → Type ℓ′}
     {x y : A}
     (f : (a : A) → B a)
     (p : x         ＝              y)
   → ＜ f x ／ (λ i → B (p i)) ＼ f y ＞
...
fibre : {ℓ ℓ′ : Level} {A : Type ℓ} {B : Type ℓ′} (f : A → B) (y : B) → Type (ℓ l⊔ ℓ′)
